...
GOCLI=${GOCLI:-${KUBEVIRTCI_PATH}/cli.sh}
KUBE_URL=${KUBE_URL:-""}
MAROONEDPODS_NAMESPACE=${MAROONEDPODS_NAMESPACE:-maroonedpods}


KUBECTL_SH="${KUBEVIRTCI_PATH}/kubectl.sh"
OPERATOR_CONTAINER_IMAGE=$(${KUBECTL_SH} get deployment -n $MAROONEDPODS_NAMESPACE maroonedpods-operator -o'custom-columns=spec:spec.template.spec.containers[0].image' --no-headers)
...
retry_counter=0
while [ $retry_counter -lt $MAX_MAROONEDPODS_WAIT_RETRY ] && [ -n "$(${KUBECTL_SH} get pods -n $MAROONEDPODS_NAMESPACE -o'custom-columns=status:status.containerStatuses[*].ready' --no-headers | grep false)" ]; do
    ...
    if [ $retry_counter -gt 1 ] && [ "$((retry_counter % 6))" -eq 0 ]; then
        ${KUBECTL_SH} get pods -n $MAROONEDPODS_NAMESPACE
    fi
done
...
if [ $retry_counter -eq $MAX_MAROONEDPODS_WAIT_RETRY ]; then
    ...
    ${KUBECTL_SH} get pods -n $MAROONEDPODS_NAMESPACE
    ${KUBECTL_SH} get pods -n $MAROONEDPODS_NAMESPACE -o yaml
    ${KUBECTL_SH} describe pods -n $MAROONEDPODS_NAMESPACE
    exit 1
fi
