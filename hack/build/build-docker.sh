...
PUSH_TARGETS=(${PUSH_TARGETS:-$CONTROLLER_IMAGE_NAME $MAROONEDPODS_SERVER_IMAGE_NAME $OPERATOR_IMAGE_NAME $SHIM_IMAGE_NAME $VSOCKFWD_IMAGE_NAME})
...
for target in ${PUSH_TARGETS[@]}; do
    ...
    if [ "${opt}" == "build" ]; then
        (
            ...
            if [ -n "${CACHEBUST:-}" ]; then
                cache+=(--build-arg "CACHEBUST=${CACHEBUST}")
            fi
