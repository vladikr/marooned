...
cluster-up:
	@if [ -n "$${KUBEVIRT_DIR}" ]; then \
		echo "KUBEVIRT_DIR is set; the cluster belongs to that KubeVirt tree."; \
		echo "  cd $${KUBEVIRT_DIR} && KUBEVIRT_MEMORY_SIZE=$${KUBEVIRT_MEMORY_SIZE:-9216M} make cluster-up && make cluster-sync"; \
